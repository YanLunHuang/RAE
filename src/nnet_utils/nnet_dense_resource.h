...
void dense_resource_rf_leq_nin_d3(int count,
                               data_T data[CONFIG_T::n_in], res_T res[CONFIG_T::n_out*3],
                               typename CONFIG_T::weight_t weights_v1[CONFIG_T::n_in * CONFIG_T::n_out],
                               typename CONFIG_T::weight_t weights_v2[CONFIG_T::n_in * CONFIG_T::n_out],
                               typename CONFIG_T::weight_t weights_v3[CONFIG_T::n_in * CONFIG_T::n_out],
                               typename CONFIG_T::bias_t biases[CONFIG_T::n_out*3]) {
    ...
    typename CONFIG_T::weight_t weights_com[CONFIG_T::n_in * CONFIG_T::n_out];
    ...
    #pragma HLS function_instantiate variable=weights_v1,weights_v2,weights_v3,biases
    //#pragma HLS RESOURCE variable=weights core=RAM_2P_BRAM Commenting out the deisgnation HLS seems to choose correctly
    #pragma HLS ARRAY_RESHAPE   variable=weights_com block factor=block_factor
    #pragma HLS ARRAY_RESHAPE   variable=weights_v1 block factor=block_factor
    #pragma HLS ARRAY_RESHAPE   variable=weights_v2 block factor=block_factor
    #pragma HLS ARRAY_RESHAPE   variable=weights_v3 block factor=block_factor
...
InitAccum:
    for (int iacc = 0; iacc < nout; iacc++) {
        #pragma HLS UNROLL
        if(count==0)acc[iacc] = (typename CONFIG_T::accum_t)biases[iacc];
        else if(count==0)acc[iacc] = (typename CONFIG_T::accum_t)biases[iacc+nout];
        else acc[iacc] = (typename CONFIG_T::accum_t)biases[iacc+2*nout];
    }
   /* 
    InitRes:
    for (int iacc = 0; iacc < 2*nout; iacc++) {
        #pragma HLS PIPELINE
        if(count==0)res[iacc] = 0;
    }*/
...
ReuseLoop:
    ...
    MultLoop:
        for (int im = 0; im < block_factor; im++) {
            ...
            if(count == 0)weights_com[w_index] = weights_v1[w_index];
            else if(count==1)weights_com[w_index] = weights_v2[w_index];
            else weights_com[w_index] = weights_v3[w_index];
            acc[out_index] += static_cast<typename CONFIG_T::accum_t>(
                CONFIG_T::template product<data_T, typename CONFIG_T::weight_t>::product(data[in_index], weights_com[w_index]));
...
Result: for (int ires = 0; ires < CONFIG_T::n_out; ires++) {
            #pragma HLS PIPELINE
            if(count==0)res[ires] = cast<data_T, res_T, CONFIG_T>(acc[ires]);
            else if(count==1)res[ires+CONFIG_T::n_out] = cast<data_T, res_T, CONFIG_T>(acc[ires]);
            else res[ires+2*CONFIG_T::n_out] = cast<data_T, res_T, CONFIG_T>(acc[ires]);
        }
}
