...
void dense_resource_rf_leq_nin(int count,
    ...
    #pragma HLS function_instantiate variable=weights_v1,weights_v2,biases
...
InitAccum:
    for (int iacc = 0; iacc < nout; iacc++) {
        ...
        else acc[iacc] = (typename CONFIG_T::accum_t)biases[iacc+nout];
    }
   /* 
    InitRes:
...
ReuseLoop:
    ...
    MultLoop:
        for (int im = 0; im < block_factor; im++) {
            ...
            else weights_com[w_index] = weights_v2[w_index];
...
Result: for (int ires = 0; ires < CONFIG_T::n_out; ires++) {
            ...
            else res[ires+CONFIG_T::n_out] = cast<data_T, res_T, CONFIG_T>(acc[ires]);
        }
							   }
