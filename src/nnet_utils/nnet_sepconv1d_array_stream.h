...
void pointwise_mult_buffer_single(
...
template<class data_T, class res_T, typename CONFIG_T>
void pointwise_conv_1d_cl_single(
    ...
    ReadInputWidth: for (unsigned i_iw = 0; i_iw < CONFIG_T::in_width; i_iw++) {
        ...
        if (i_iw % CONFIG_T::stride_width == 0) {
            ...
            for(int j = 0; j < CONFIG_T::n_chan; j++){
                #pragma HLS PIPELINE
                data_store[j] = data[0].read();
            }
            pointwise_mult_buffer_single<data_T, res_T, CONFIG_T>(data_store, res, weights, biases);
        } else {
            for(int j = 0; j < CONFIG_T::n_chan; j++){
                #pragma HLS PIPELINE
                data_store[j] = data[0].read();
...
template<class data_T, class res_T, typename CONFIG_T>
void pointwise_conv_1d_cl_array(
