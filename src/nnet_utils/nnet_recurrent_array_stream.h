...
template<class data_T, class init_T, class res_T, typename CONFIG_T>
  void gru_stack_single(
      hls::stream<data_T> data_stream[1],
    ...
    typename CONFIG_T::weight_t     param_com   [CONFIG_T::n_state*3*CONFIG_T::n_in];
    typename CONFIG_T::weight_t     param_zr_com[CONFIG_T::n_state*3*CONFIG_T::n_state];
    typename CONFIG_T::bias_t       param_b_com [CONFIG_T::n_state*3];
    typename CONFIG_T::bias_t       param_br_com [CONFIG_T::n_state*3];

    for(int i=0; i<CONFIG_T::n_state*3*CONFIG_T::n_in;i++) param_com[i] = param[i];
    for(int i=0; i<CONFIG_T::n_state*3*CONFIG_T::n_state;i++) param_zr_com[i] = param_zr[i];
    for(int i=0; i<CONFIG_T::n_state*3;i++) param_b_com[i] = param_b[i];
    for(int i=0; i<CONFIG_T::n_state*3;i++) param_br_com[i] = param_br[i];
    ...
    DataPropagation: for(int i_in = 0; i_in < CONFIG_T::n_sequence; i_in++) {
      ...
      DataPack: for (int i_pack = 0; i_pack < CONFIG_T::n_in; i_pack++) {
          #pragma HLS PIPELINE
          data_in[i_pack] = data_stream[0].read();
      }
      if (CONFIG_T::use_static)
        nnet::gru_static<data_T, res_T, CONFIG_T>(reset_state,data_in,h_newstate,param_com,param_zr_com,param_b_com, param_br_com);
      else
        nnet::gru<data_T, res_T, CONFIG_T>(reset_state,data_in,h_newstate,param_com,param_zr_com,param_b_com, param_br_com);
